...
type PreparedConceptContext = {
  premarketPDA: string;
  premarketId: string;
  effectiveWhitelist: WhitelistData;
};
...
async function prepareConcept(
  ...
  await Promise.all([
    ...
    vestingData
      ? updateVestingInfo(conceptResp.premarket_account_pda, wallet.publicKey.toBase58(), {
  ...
  console.log("[prepareConcept] whitelist payload", {
  ...
  return {
    ...
    premarketId: String(conceptResp.premarket_id),
    effectiveWhitelist,
  };
}

export async function createPremarketConcept(
  network: "devnet" | "mainnet-beta",
  wallet: AnchorWallet,
  args: CreatePremarketArgs,
  communityInfo: AddCommunityInfoParams,
...
export async function launchPremarketFromConcept(
  network: "devnet" | "mainnet-beta",
  wallet: AnchorWallet,
  ...
  return createPremarketInBlockchain(
    network,
    wallet,
    connection,
    {
      premarketAccountPda: premarketPDA,
...
export async function createPremarket(
  ...
  const concept = await createPremarketConcept(
    network,
    wallet,
    args,
    communityInfo,
  ...
  return launchPremarketFromConcept(
    network,
    wallet,
    connection,
    args,
    concept.premarketPDA,
    onChangeState
  );
}
